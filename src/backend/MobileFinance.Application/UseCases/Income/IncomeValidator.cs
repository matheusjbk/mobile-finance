...
public class IncomeValidator : AbstractValidator<RequestIncomeJson>
{
    public IncomeValidator()
    {
        ...
        When(income => income.IncomeType.Equals(IncomeType.OneTime) || income.IncomeType.Equals(IncomeType.Rent), () => 
            RuleFor(income => income.ReceivedOn).NotEmpty().WithMessage(ExceptionMessages.EMPTY_RECEIVED_DATE));
        When(income => income.IncomeType.Equals(IncomeType.Rent), () =>
            RuleFor(income => income.RecurrenceMonthsCount).NotEmpty());
       When(income => income.IncomeType.Equals(IncomeType.Salary) || income.IncomeType.Equals(IncomeType.Rent), () =>
        ...
        When(income => income.IncomeType.Equals(IncomeType.Salary), () =>
            RuleFor(income => income.BusinessDayNumber).NotEmpty());
    }
}
