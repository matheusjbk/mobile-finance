...
namespace WebApi.Test.Income.Delete;
public class DeleteIncomeInvalidTokenTest : MobileFinanceClassFixture
{
    private const string ROUTE = "income";

    public DeleteIncomeInvalidTokenTest(MobileFinanceWebApplicationFactory factory) : base(factory) { }
    ...
    public async Task Error_Invalid_Token(string culture)
    {
        ...
        var response = await DoDelete(route: $"{ROUTE}/{id}", token: "tokenInvalid", culture: culture);
    ...
    {
        var id = IdEncoderBuilder.Build().Encode(1);
        var response = await DoDelete(route: $"{ROUTE}/{id}", token: string.Empty, culture: culture);
    ...
    public async Task Error_Valid_Token_User_NotFound(string culture)
    {
        ...
        var id = IdEncoderBuilder.Build().Encode(1);
        var response = await DoDelete(route: $"{ROUTE}/{id}", token: token, culture: culture);
