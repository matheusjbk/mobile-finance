...
public class GetUserProfileInvalidTokenTest : MobileFinanceClassFixture
{
    private const string ROUTE = "user";

    public GetUserProfileInvalidTokenTest(MobileFinanceWebApplicationFactory factory) : base(factory) { }

    [Theory]
    [ClassData(typeof(CultureInlineDataTest))]
    public async Task Error_Invalid_Token(string culture)
    {
        var response = await DoGet(route: ROUTE, token: "tokenInvalid", culture: culture);
